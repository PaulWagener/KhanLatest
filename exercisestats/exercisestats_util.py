...
from exercise_models import ProblemLog, Exercise
...
def enqueue_task(exid, start_dt, end_dt, cursor, uid, i):
    ...
    try:
        key_name = ExerciseStatisticShard.make_key(exid, start_dt, end_dt,
                                                   cursor)
        ...
        logging.info("Ignoring task named '%s' as it already exists",
                     task_name)
...
def fancy_stats_from_logs(problem_logs):
    ...
    for problem_log in problem_logs:
        ...
        if problem_log.earned_proficiency:
            ...
            prof_freq_table[problem_num] = \
                    1 + prof_freq_table.get(problem_num, 0)

        if (problem_log.random_float < IP_ADDRESS_SAMPLE_RATE and
                problem_log.ip_address):
...
def fancy_stats_shard_reducer(exid, start_dt, end_dt):
    ...
    # log_count can't be just a normal variable because Python
    # closures are confusing http://stackoverflow.com/questions/4851463
    results = {
        'log_count': 0,
        'new_user_count': 0,
